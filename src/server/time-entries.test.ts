...
describe('updateTimeEntry', () => {
  it("rejects when an employee tries to edit another employee's entry", async () => {
    ...
    prismaMock.timeEntry.findFirst.mockResolvedValue(makeEntry({ userId: 'other' }))
  ...
  it('allows an admin to edit any entry', async () => {
    ...
    prismaMock.timeEntry.findFirst.mockResolvedValue(makeEntry({ userId: 'someone-else' }))
  ...
  it('recomputes weekEnding when workDate changes', async () => {
    ...
    prismaMock.timeEntry.findFirst.mockResolvedValue(makeEntry({ userId: 'u1' }))
    prismaMock.timeEntry.update.mockResolvedValue(makeEntry())
...
describe('deleteTimeEntry', () => {
  ...
  it('allows an admin to delete any entry', async () => {
    requireUserMock.mockResolvedValue(makeUser({ id: 'admin', role: 'ADMIN' }))
    prismaMock.timeEntry.findFirst.mockResolvedValue(makeEntry({ userId: 'someone' }))
  ...
  it('allows an employee to delete their own entry', async () => {
    requireUserMock.mockResolvedValue(makeUser({ id: 'u1', role: 'EMPLOYEE' }))
    prismaMock.timeEntry.findFirst.mockResolvedValue(makeEntry({ userId: 'u1' }))
